...
sed -e "s/BGCOLOR/$background/g" \
    ...
    -e "s/COLOR8/$color8/g" \
    -e "s/COLOR9/$color9/g" \
    ~/.local/bin/pywal-templates/pywal-custom-template.xmobar > ~/.config/xmobar/pywal-custom-xmobarrc
...
sed -e "s/BGCOLOR/$background/g" \
    ...
    -e "s/COLOR9/$color9/g" \
    -e "s/0x#/0x/g" \
    ~/.local/bin/pywal-templates/pywal-custom-template.xmonad > ~/.config/xmonad/lib/Colors/PyWalCustom.hs